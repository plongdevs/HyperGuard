...
from HyperGuard import util
from HyperGuard.instruction import BinaryCompExpression, Constant
from HyperGuard.opcode_ins import Op
from HyperGuard.util import get_type_descriptor, get_native_type, JniLongName, is_primitive_type, \
    get_cdecl_type, get_type

logger = logging.getLogger('HyperGuard.writer')

# Helper: bọc class name trong AY_OBFUSCATE cho generated C++
def _obf(s):
    return 'AY_OBFUSCATE("%s")' % s
...
class Writer(object):
    ...
    def write_method(self):
        ...
        # Fix: comment bị xóa — không để lộ class/method name trong .so
    ...
    def visit_landing_pad(self, landing_pad):
        ...
        for atype, handle in landing_pad.handles.items():
            # Fix: exception class name obfuscated
    ...
    def visit_put_static(self, ins, clsdesc, name, ftype, rhs):
        ...
        self.write('D2C_RESOLVE_STATIC_FIELD(clz, fld, %s, "%s", "%s");\n' % (
            _obf(get_type(clsdesc)), name, ftype))
    ...
    def visit_put_instance(self, ins, lhs, rhs, ftype, clsdesc, name):
        ...
        self.write('env->Set%sField(v%s,fld,(%s) %s);\n' % (
            get_type_descriptor(ftype), self.ra(lhs), get_native_type(ftype),
    ...
    def _invoke_common(self, ins, invoke_type, name, base, ptype, rtype, args, clsdesc):
        ...
        # Fix: class name obfuscated, method name/sig left as-is
        if invoke_type != 'static':
            self.write('D2C_RESOLVE_METHOD(clz, mid, %s, "%s", "(%s)%s");\n' % (
                _obf(get_type(clsdesc)), name, ''.join(ptype), rtype))
        else:
    ...
    def visit_new_array(self, ins, result, atype, size):
        ...
        if is_primitive_type(elem_type):
            ...
            self.write(' = (%s) env->New%sArray((jint) %s);\n' % (
                get_native_type(result.get_type()), get_type_descriptor(elem_type),
                self.get_variable_or_const(size)))
        else:
            self.write('jclass &clz = %s;\n' % (self.ca(ins.get_class())))
            # Fix: class name obfuscated
            self.write('D2C_RESOLVE_CLASS(clz, %s);\n' % _obf(get_type(elem_type)))
            result.visit(self)
    ...
    def visit_filled_new_array(self, ins, result, atype, size, args):
        self.write_trace(ins)
        params = ['%s' % (self.get_variable_or_const(arg)) for arg in args]
        self.write('{\n')
        self.write_define_ex_handle(ins)
        self.write_kill_local_reference(ins.get_value())
        elem_type = atype[1:]
    ...
    def visit_fill_array(self, ins, array, value):
        ...
        self.write('env->Set%sArrayRegion((%sArray) v%s, 0, %d, (const %s *) data);\n' % (
            get_type_descriptor(elem_type), get_native_type(elem_type),
            self.ra(array), array_size, get_native_type(elem_type)))
        self.write('}\n')
