...
cd $HOME
if [ -d "HyperGuard" ]; then
  cd HyperGuard
elif [ -f "HyperGuard.py" ] && [ -d "tools" ]; then
  :
else
  git clone https://github.com/TechnoIndian/HyperGuard || exit 2
  cd HyperGuard || exit 2
fi

if [ -f "$HOME/HyperGuard/tools/apktool.jar" ]; then
  rm $HOME/HyperGuard/tools/apktool.jar
  cp $PREFIX/bin/apktool.jar $HOME/HyperGuard/tools/apktool.jar
else
sh -c 'wget https://bitbucket.org/iBotPeaches/apktool/downloads/apktool_2.9.3.jar -O $HOME/HyperGuard/tools/apktool.jar'
fi
...
cat > $HOME/HyperGuard/HyperGuard.cfg << EOL
{
...
echo "${green}============================"
echo "Great! HyperGuard installed successfully!"
echo "============================${nocolor}"
